...
class StubOfferService:
    ...
    def __init__(
        ...
        db_session=None,
        logger=None,
        log_service: Optional[StubLogService] = None,
    ):
    ...
    if current_user_data and current_user_data.get("role") == UserRole.SELLER:
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "message": "Only sellers can perform this operation.",
            },
        )
...
@pytest.mark.parametrize(
    "error_status, error_code, error_message, service_exception_detail_dict",
    [
        (
        ...
        (
            ...
            "Cannot mark offer with status 'archived' as sold",
            {
        ...
        (
            status.HTTP_409_CONFLICT,
            "ALREADY_SOLD",
            "Offer is already marked as sold",
            {
        ...
        (
            ...
            "NOT_OFFER_OWNER",
            "You can only mark your own offers as sold",
            {
...
def test_mark_offer_as_sold_service_errors(
    seller_auth,
    error_status,
    error_code,
    error_message,
...
def test_mark_offer_as_sold_server_error(seller_auth):
    ...
    assert "An unexpected error occurred" in response_json["detail"].get(
        "message", ""
    )
...
def test_mark_offer_as_sold_not_authenticated(no_auth):
    ...
    assert isinstance(response_json["detail"], dict)
    assert (
        response_json["detail"].get("error_code") == "INSUFFICIENT_PERMISSIONS"
    )
    assert "Only sellers can perform this operation." in response_json[
        "detail"
    ].get("message", "")
...
def get_offer_service_override():
    mock_log_service_instance = (
        StubLogService()
    )  # Tworzymy instancję mocka LogService
...
def test_create_offer_not_authenticated(no_auth):
    ...
    assert (
        "Only sellers can perform this operation"
...
def test_deactivate_offer_success(seller_auth):
    ...
    assert log_entry["event_type"] == LogEventType.OFFER_STATUS_CHANGE.value
    assert (
        f"Offer {MOCK_OFFER_ID} status changed to inactive"
...
def test_deactivate_offer_invalid_status_transition(seller_auth):
    ...
    assert (
        "Cannot deactivate offer with status 'sold'"
        in response_json["detail"]["message"]
    )
...
def test_deactivate_offer_not_owner(seller_auth):
    ...
    assert (
        "You can only deactivate your own offers"
        in response_json["detail"]["message"]
    )
...
async def api_create_offer(
    ...
    if (
        ...
        or current_user_data.get("role") != UserRole.SELLER
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={
