...
from types import SimpleNamespace # Added for potential use, though might not be needed now
...
app.dependency_overrides[dependencies.require_authenticated] = _authenticated_session
...
class StubUserService:
    ...
    @classmethod
    async def _default_update_user_profile(cls, user_id: UUID, update_data: UpdateUserRequest):
         return UserDTO(
            id=user_id,
            email=MOCK_USER_EMAIL,
            role='Buyer',
            status='Active',
            first_name=update_data.first_name or 'John', # Use original if None
    ...
    @classmethod
    async def _default_change_password(cls, user_id: UUID, password_data: ChangePasswordRequest, request_ip: str):
    ...
    async def update_user_profile(self, user_id: UUID, update_data: UpdateUserRequest):
        StubUserService.update_user_profile_called = True
        StubUserService.update_user_profile_called_with = (user_id, update_data)
        return await StubUserService._update_user_profile_impl(user_id, update_data)

    async def change_password(self, user_id: UUID, password_data: ChangePasswordRequest, request_ip: str):
        StubUserService.change_password_called = True
        StubUserService.change_password_called_with = (user_id, password_data, request_ip)
        return await StubUserService._change_password_impl(user_id, password_data, request_ip)
...
def manage_stubs(monkeypatch):
    ...
    monkeypatch.setitem(app.dependency_overrides, account_router.CsrfProtect, lambda: NoopCsrfProtect())
    monkeypatch.setitem(app.dependency_overrides, dependencies.require_authenticated, _authenticated_session)
    yield
...
def test_get_current_user_success():
    ...
    assert data['updated_at'].startswith('2023-01-02T13:00:00') # Matches default stub
...
def test_get_current_user_not_found(monkeypatch):
    ...
    async def raise_not_found(*args, **kwargs): # Use generic signature for patching
...
def test_get_current_user_server_error(monkeypatch):
    ...
    # Patch the main method directly
    monkeypatch.setattr(StubUserService, 'get_current_user', raise_server_error)
    ...
    assert body['detail']['message'] == 'Wystąpił nieoczekiwany błąd podczas pobierania profilu użytkownika.'

# ==================================================
# === Tests for PATCH /account Endpoint ===
# ==================================================

@pytest.mark.parametrize('payload, expected_first, expected_last', [
...
def test_update_profile_success(payload, expected_first, expected_last):
    ...
    assert data['updated_at'].startswith('2023-01-03T14:00:00') # Matches default stub update time
...
def test_update_profile_unauthenticated(monkeypatch):
    # Override authentication to raise for this test
    def bad_auth():
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail={'error_code':'NOT_AUTHENTICATED','message':'Użytkownik nie jest zalogowany.'})
    monkeypatch.setitem(app.dependency_overrides, dependencies.require_authenticated, bad_auth)
...
def test_update_profile_not_found(monkeypatch):
    ...
    async def raise_not_found(*args, **kwargs):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail={'error_code':'USER_NOT_FOUND','message':'Nie znaleziono użytkownika.'})
    # Patch the main method directly
    monkeypatch.setattr(StubUserService, 'update_user_profile', raise_not_found)
...
def test_update_profile_server_error(monkeypatch):
    ...
    # Patch the main method directly
    monkeypatch.setattr(StubUserService, 'update_user_profile', raise_server_error)
    ...
    assert 'Wystąpił błąd podczas aktualizacji profilu użytkownika.' in detail.get('message', '')

# =====================================================
# === Tests for PUT /account/password Endpoint ===
# =====================================================

valid_password_payload = {"current_password": "OldPass123!", "new_password": "NewValidPass456!"}
...
@pytest.mark.parametrize('payload, expected_error_part', [
    ({}, 'Field required'), # Missing fields
    ({"current_password": "abc"}, 'Field required'), # Missing new_password
    ({"new_password": "abc"}, 'Field required'), # Missing current_password
    # Password policy checks (assuming they are in the Pydantic model)
    ({"current_password": "OldPass123!", "new_password": "short"}, 'Password must be at least 10 characters long'),
    ({"current_password": "OldPass123!", "new_password": "nouppercase123!"}, 'Password must contain an uppercase letter'),
    ({"current_password": "OldPass123!", "new_password": "NOLOWERCASE123!"}, 'Password must contain a lowercase letter'),
    ({"current_password": "OldPass123!", "new_password": "NoDigitOrSpecial"}, 'Password must contain a digit or special character'),
])
def test_change_password_pydantic_validation_error(payload, expected_error_part):
...
def test_change_password_invalid_current(monkeypatch):
    ...
    async def raise_invalid_current(*args, **kwargs):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail={'error_code':'INVALID_CURRENT_PASSWORD', 'message':'Aktualne hasło jest nieprawidłowe.'})
    # Patch the main method directly
    monkeypatch.setattr(StubUserService, 'change_password', raise_invalid_current)
...
def test_change_password_user_not_found(monkeypatch):
    ...
    response = client.put('/account/password', json=valid_password_payload)
    assert response.status_code == status.HTTP_404_NOT_FOUND # Assuming service raises 404
    body = response.json()
    detail = body.get('detail', {})
    assert detail.get('error_code') == 'USER_NOT_FOUND'
