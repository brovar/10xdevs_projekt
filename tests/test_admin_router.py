...
from schemas import (AdminLogListQueryParams, AdminOfferListQueryParams,
                     AdminOrderListQueryParams, CategoryDTO, LogEventType,
                     LogListResponse, OfferDetailDTO, OfferListResponse,
                     OfferStatus, OrderDetailDTO, OrderItemDTO,
                     OrderListResponse, OrderStatus, OrderSummaryDTO,
                     SellerInfoDTO, UserDTO, UserListQueryParams,
...
class MockDBSession:
    ...
    async def get(self, model_class, item_id):
        ...
        if (
            hasattr(model_class, "_mock_instances")
            and item_id in model_class._mock_instances
        ):
...
class StubUserService:
    ...
    def _record_call(self, method_name, **kwargs):
        ...
        StubUserService._call_count[method_name] = (
            StubUserService._call_count.get(method_name, 0) + 1
        )
    ...
    async def list_users(
        self, page=1, limit=10, role=None, status=None, search=None
    ):
        self._record_call(
            "list_users",
            page=page,
...
class StubLogService:
    ...
    def _record_call(self, method_name, **kwargs):
        ...
        StubLogService._call_count[method_name] = (
            StubLogService._call_count.get(method_name, 0) + 1
        )
...
class StubOfferService:
    ...
    def _record_call(self, method_name, **kwargs):
        ...
        StubOfferService._call_count[method_name] = (
            StubOfferService._call_count.get(method_name, 0) + 1
        )
    ...
    async def moderate_offer(self, offer_id: UUID):
        ...
        mock_seller = SellerInfoDTO(
            id=MOCK_SELLER_ID, first_name="Seller", last_name="Name"
        )

        mock_category = CategoryDTO(id=1, name="Test Category")
...
class StubOrderService:
    ...
    async def get_admin_orders(
        self, page=1, limit=10, status=None, buyer_id=None, seller_id=None
    ):
        self._record_call(
            "get_admin_orders",
            page=page,
...
MOCK_USER_LIST_RESPONSE = UserListResponse(
    items=[MOCK_USER_DTO], total=1, page=1, limit=10, pages=1
)
...
def override_dependencies(monkeypatch):
    ...
    stub_offer_service = StubOfferService(
        db_session=mock_session, logger=logger
    )
    stub_order_service = StubOrderService(
        db_session=mock_session, logger=logger
    )
    ...
    app.dependency_overrides[admin_router.LogService] = (
        lambda: stub_log_service
    )
    app.dependency_overrides[admin_router.OfferService] = (
        lambda: stub_offer_service
    )
    app.dependency_overrides[admin_router.OrderService] = (
        lambda: stub_order_service
    )

    # Override query parameter validation classes
    monkeypatch.setattr(
        admin_router, "UserListQueryParams", MockUserListQueryParams
    )
    monkeypatch.setattr(
        admin_router,
        "AdminOfferListQueryParams",
        MockAdminOfferListQueryParams,
    )
    monkeypatch.setattr(
        admin_router,
        "AdminOrderListQueryParams",
        MockAdminOrderListQueryParams,
    )
    monkeypatch.setattr(
        admin_router, "AdminLogListQueryParams", MockAdminLogListQueryParams
    )
...
def test_list_users_with_filters():
    ...
    response = client.get(
        "/admin/users?role=Seller&status=Active&search=test&page=2&limit=20"
    )
