...
from schemas import (
    UserRole, LogEventType, OrderStatus, TransactionStatus,
...
class StubPaymentService:
    ...
    def _record_call(self, method_name, **kwargs):
        ...
        StubPaymentService._call_count[method_name] = StubPaymentService._call_count.get(method_name, 0) + 1

    def _maybe_raise(self):
        if StubPaymentService._raise:
            raise StubPaymentService._raise

    async def process_payment_callback(self, transaction_id: UUID, status: TransactionStatus):
        self._record_call('process_payment_callback', transaction_id=transaction_id, status=status)
        self._maybe_raise()
...
def override_dependencies():
    ...
    app.dependency_overrides[dependencies.get_payment_service] = lambda: stub_payment_service
...
def test_payment_callback_success_status():
    ...
    response = client.get(f"/payments/callback?transaction_id={transaction_id}&status_str=success")
    ...
    # Verify service call
    assert StubPaymentService._call_count.get('process_payment_callback', 0) == 1
...
def test_payment_callback_fail_status():
    ...
    transaction_id = str(MOCK_TRANSACTION_ID)
    response = client.get(f"/payments/callback?transaction_id={transaction_id}&status_str=fail")
...
def test_payment_callback_cancelled_status():
    ...
    transaction_id = str(MOCK_TRANSACTION_ID)
    response = client.get(f"/payments/callback?transaction_id={transaction_id}&status_str=cancelled")
...
def test_payment_callback_missing_transaction_id():
    ...
    assert StubPaymentService._call_count.get('process_payment_callback', 0) == 0
...
def test_payment_callback_missing_status():
    ...
    transaction_id = str(MOCK_TRANSACTION_ID)
    response = client.get(f"/payments/callback?transaction_id={transaction_id}")
...
def test_payment_callback_invalid_status():
    ...
    transaction_id = str(MOCK_TRANSACTION_ID)
    response = client.get(f"/payments/callback?transaction_id={transaction_id}&status_str=invalid_status")

    # Expect 400 BAD REQUEST
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    data = response.json()
    assert data["detail"]["error_code"] == "INVALID_STATUS"
    assert "Allowed values: success, fail, cancelled" in data["detail"]["message"]
...
def test_payment_callback_invalid_transaction_id():
    ...
    response = client.get("/payments/callback?transaction_id=not-a-uuid&status_str=success")
...
def test_payment_callback_transaction_not_found():
    ...
    StubPaymentService._raise = ValueError(f"Transaction {transaction_id} not found")
    ...
    assert f"Transaction {transaction_id} not found" in data["detail"]["message"]
...
def test_payment_callback_order_not_found():
    ...
    transaction_id = str(MOCK_TRANSACTION_ID)
    StubPaymentService._raise = ValueError(f"Order not found for transaction {transaction_id}")
    ...
    assert f"Order not found for transaction {transaction_id}" in data["detail"]["message"]
    ...
    assert log_entry['event_type'] == LogEventType.PAYMENT_CALLBACK_FAIL
    assert "Payment callback failed" in log_entry['message']
    assert f"Order not found for transaction {transaction_id}" in log_entry['message']

def test_payment_callback_order_already_processed():
    """Test callback when order is already processed."""
    transaction_id = str(MOCK_TRANSACTION_ID)
    StubPaymentService._raise = ConflictError("Order has already been processed")
...
def test_payment_callback_service_error():
    ...
    log_entry = StubLogService.logs[0]
    assert log_entry['event_type'] == LogEventType.PAYMENT_CALLBACK_FAIL
    assert "Unexpected error processing payment callback" in log_entry['message']
...
def test_payment_callback_success_logging():
    ...
    log_entry = StubLogService.logs[0]
    assert log_entry['event_type'] == LogEventType.PAYMENT_CALLBACK_SUCCESS
    assert f"Successfully processed payment callback for transaction {transaction_id}" in log_entry['message']
    assert "fail" in log_entry['message']
