...
class SuccessRegisterAuthService(BaseStubAuthService):
    async def register_user(self, register_data, request):
        # Print debug info
        print(
            f"SuccessRegisterAuthService.register_user called with: {register_data}"
        )
...
def auth_client(test_app, auth_service_class):
    ...
    test_app.dependency_overrides[dependencies.get_auth_service] = (
        lambda: auth_service_class()
    )
...
def test_register_user_success(test_app):
    ...
    test_app.dependency_overrides[dependencies.get_auth_service] = (
        lambda: SuccessRegisterAuthService()
    )
...
def test_register_user_conflict(test_app):
    ...
    test_app.dependency_overrides[dependencies.get_auth_service] = (
        lambda: ConflictRegisterAuthService()
    )
...
def test_register_user_unexpected_error(test_app):
    ...
    test_app.dependency_overrides[dependencies.get_auth_service] = (
        lambda: ErrorRegisterAuthService()
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"email": "bad-email", "password": "short", "role": "Buyer"},
            ["email", "password"],
...
def test_register_user_validation_errors(
    test_app, payload, expected_substrings
):
...
def test_login_success(test_app, payload):
    ...
    test_app.dependency_overrides[dependencies.get_auth_service] = (
        lambda: SuccessLoginAuthService()
    )
...
def test_login_invalid_credentials(test_app):
    ...
    test_app.dependency_overrides[dependencies.get_auth_service] = (
        lambda: InvalidCredsLoginAuthService()
    )
...
def test_login_user_inactive(test_app):
    ...
    test_app.dependency_overrides[dependencies.get_auth_service] = (
        lambda: InactiveUserLoginAuthService()
    )
...
def test_logout_success(test_app):
    ...
    test_app.dependency_overrides[dependencies.get_auth_service] = (
        lambda: SuccessLogoutAuthService()
    )
...
def test_logout_unexpected_error(test_app):
    ...
    test_app.dependency_overrides[dependencies.get_auth_service] = (
        lambda: ExceptionLogoutAuthService()
    )
    ...
    class CsrfFailureProtect:
        def validate_csrf(self, request):
            ...
            raise CsrfProtectError(
                status_code=403, message="Invalid CSRF token"
            )
