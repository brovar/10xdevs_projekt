...
from fastapi import status, Request, BackgroundTasks, UploadFile, File, Form, HTTPException
...
from schemas import UserRole, OfferStatus, LogEventType
...
def create_test_image(format_type='jpeg', size_kb=100):
    ...
    upload_file = UploadFile(file=file, filename=f"test_image{ext}", headers=headers)
...
class MockOfferService:
    ...
    async def create_offer(self, seller_id, title, price, category_id, quantity=1, 
                          description=None, image: Optional[UploadFile] = None, # Added type hint for image
        ...
        if category_id == 9999: # Specific ID used in test_offer_nonexistent_category
            raise HTTPException(
        ...
        if price > 1000000: # Example upper limit for price from test
             raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error_code": "INVALID_PRICE", "message": "Price exceeds maximum limit of 1,000,000"}
            )
        if quantity <= 0: # Changed from <0 to <=0 to match test which tests quantity 0
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error_code": "INVALID_QUANTITY", "message": "Quantity must be greater than zero"} # Message from test
            )
        if quantity > 1000:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error_code": "INVALID_QUANTITY", "message": "Quantity cannot exceed 1000"}
            )
        if len(title) < 3:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error_code": "INVALID_TITLE", "message": "Title must be at least 3 characters"}
            )
        if len(title) > 100:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error_code": "INVALID_TITLE", "message": "Title cannot exceed 100 characters"}
            )
        if description and len(description) > 2000:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail={"error_code": "INVALID_DESCRIPTION", "message": "Description cannot exceed 2000 characters"}
            )
...
class MockFileService:
    ...
    async def save_image(self, image: UploadFile):
        ...
        if image.content_type not in allowed_types:
            ...
            if image.content_type in ['image/gif', 'image/svg+xml', 'image/bmp']:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
...
def setup_dependencies(monkeypatch):
    ...
    app.dependency_overrides[dependencies.require_seller] = lambda: mock_seller()
    app.dependency_overrides[dependencies.get_log_service] = lambda: MockLogService(logger=test_logger) # Ensure get_log_service also gets a logger
    
    # Modify get_offer_service override to inject MockFileService
    app.dependency_overrides[dependencies.get_offer_service] = lambda: MockOfferService(
        logger=test_logger, 
        file_service=mock_file_service_instance
    )
    app.dependency_overrides[offer_router.CsrfProtect] = lambda: MockCsrfProtect()
...
@pytest.mark.parametrize('format_type, expected_code', [
...
def test_offer_image_format_validation(format_type, expected_code):
    ...
    # Send the request
    files = {"image": (test_image.filename, test_image.file, test_image.content_type)}
...
@pytest.mark.parametrize('size_kb, expected_code', [
...
@pytest.mark.parametrize('price, quantity, expected_code, expected_error', [
    ("0.01", "1", status.HTTP_201_CREATED, None),  # Minimum valid values
    ("0", "1", status.HTTP_400_BAD_REQUEST, "INVALID_PRICE"),  # Zero price
    ("-10", "1", status.HTTP_400_BAD_REQUEST, "INVALID_PRICE"),  # Negative price
    ("99.99", "0", status.HTTP_400_BAD_REQUEST, "INVALID_QUANTITY"),  # Zero quantity
    ("99.99", "-1", status.HTTP_400_BAD_REQUEST, "INVALID_QUANTITY"),  # Negative quantity
    ("99.99", "1001", status.HTTP_400_BAD_REQUEST, "INVALID_QUANTITY"),  # Quantity exceeds limit
    ("10000000", "1", status.HTTP_400_BAD_REQUEST, "INVALID_PRICE"),  # Price exceeds limit
])
def test_offer_numeric_field_validation(price, quantity, expected_code, expected_error):
...
@pytest.mark.parametrize('title_length, description_length, expected_code, expected_error', [
    (10, 100, status.HTTP_201_CREATED, None),  # Valid lengths
    (2, 100, status.HTTP_400_BAD_REQUEST, "INVALID_TITLE"),  # Title too short
    (101, 100, status.HTTP_400_BAD_REQUEST, "INVALID_TITLE"),  # Title too long
    (10, 2001, status.HTTP_400_BAD_REQUEST, "INVALID_DESCRIPTION"),  # Description too long
])
def test_offer_text_field_validation(title_length, description_length, expected_code, expected_error):
