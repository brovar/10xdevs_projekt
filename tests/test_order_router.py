...
from schemas import (
    UserRole, LogEventType, OrderStatus, CreateOrderResponse, 
    OrderListResponse, OrderDetailDTO, OrderItemDTO, 
    ErrorResponse, CreateOrderRequest, OrderItemRequest,
    OfferStatus
)
...
from dependencies import get_order_service, get_log_service, require_authenticated
...
app.dependency_overrides[dependencies.get_logger] = lambda: logging.getLogger('test_order')
...
class StubOrderService:
    ...
    def _record_call(self, method_name, **kwargs):
        ...
        if 'current_user' in kwargs and isinstance(kwargs['current_user'], dict):
            ...
            kwargs['current_user_role'] = kwargs['current_user'].get('user_role')
        if 'user_id' in kwargs:
            kwargs['user_uuid'] = kwargs['user_id'] # Keep raw user_id if passed directly
    ...
    async def create_order(self, current_user: Dict, order_data, ip_address):
        self._record_call('create_order', current_user=current_user, order_data=order_data, ip_address=ip_address)
        self._maybe_raise()
    ...
    async def get_buyer_orders(self, buyer_id: UUID, page, limit):
        self._record_call('get_buyer_orders', buyer_id=buyer_id, page=page, limit=limit)
        self._maybe_raise()
    ...
    async def get_order_details(self, order_id: UUID, user_id: UUID, user_role: UserRole):
        self._record_call('get_order_details', order_id=order_id, user_id=user_id, user_role=user_role)
        self._maybe_raise()
        
        # Default mock response
        mock_items = [
            OrderItemDTO(id=1, offer_id=MOCK_OFFER_ID_1, quantity=1, price_at_purchase=Decimal("50.00"), offer_title="Item 1"),
            OrderItemDTO(id=2, offer_id=MOCK_OFFER_ID_2, quantity=2, price_at_purchase=Decimal("100.00"), offer_title="Item 2")
        ]
    ...
    async def deliver_order(self, order_id: UUID, seller_id: UUID):
        self._record_call('deliver_order', order_id=order_id, seller_id=seller_id)
        self._maybe_raise()
...
def test_create_order_unauthorized(no_auth):
    ...
    assert response.json()['detail']['message'] == "Użytkownik nie jest zalogowany."
...
def test_create_order_forbidden_role(seller_auth):
    ...
    assert response.json()['detail']['message'] == "Nie masz uprawnień do wykonania tej operacji."
    assert StubOrderService._call_count.get('create_order', 0) == 0
...
def test_create_order_invalid_data():
    ...
    StubOrderService._raise = HTTPException(
        ...
        detail={"error_code": "INSUFFICIENT_QUANTITY", "message": "Not enough items"}
    )
...
def test_create_order_offer_not_found():
    ...
    payload = {"items": [{"offer_id": str(uuid4()), "quantity": 1}]} # Non-existent offer ID
    response = client.post("/orders", json=payload)
...
def test_create_order_invalid_payload_format():
    ...
    response = client.post("/orders", json=payload)
    assert response.status_code == status.HTTP_400_BAD_REQUEST # Expect 400
    assert StubOrderService._call_count.get('create_order', 0) == 0 # Service not called
...
def test_list_buyer_orders_success():
    ...
    if data["total"] > 0:
        assert data["items"][0]["id"] == str(MOCK_ORDER_ID) # Based on default stub response
...
def test_list_buyer_orders_server_error():
    ...
    assert "Failed to fetch orders: Database connection failed" in log_entry['message']
...
def test_list_buyer_orders_invalid_pagination():
    ...
    assert StubOrderService._call_count.get('get_buyer_orders', 0) == 0 # Service not called
    assert not StubLogService.logs
...
def test_get_order_details_forbidden():
    ...
    StubOrderService._raise = HTTPException(
        status_code=status.HTTP_403_FORBIDDEN,
        detail={"error_code": "ACCESS_DENIED_SERVICE", "message": "Access denied by service"} # Different error code
    )
    order_id = str(MOCK_ORDER_ID)
    response = client.get(f"/orders/{order_id}")
...
def test_get_order_details_server_error():
    ...
    assert f"Failed to fetch order details for order {order_id}: {error_message}" in log_entry['message']
...
def test_ship_order_success(seller_auth):
    ...
    assert f"Order {order_id} shipped by seller {MOCK_SELLER_ID}" in log_entry['message']
...
def test_ship_order_invalid_status(seller_auth):
    ...
    assert response.status_code == status.HTTP_409_CONFLICT # <<< THIS WILL FAIL (NameError)
    data = response.json()
    ...
    assert data['detail']['error_code'] == "INVALID_ORDER_STATUS" # <<< THIS WILL FAIL (NameError)
    assert data['detail']['message'] == error_message # <<< THIS WILL FAIL (NameError)
    assert StubOrderService._call_count.get('ship_order', 0) == 1
    ...
    assert f"Failed to ship order: {error_message}" in log_entry['message'] # <<< THIS MIGHT FAIL (NameError)
...
def test_ship_order_server_error(seller_auth):
    ...
    assert str(log_entry['user_id']) == str(MOCK_SELLER_ID)
    assert log_entry['event_type'] == LogEventType.ORDER_SHIP_FAIL
    assert f"Unexpected error shipping order {order_id}: {error_message}" in log_entry['message']
...
def test_deliver_order_success(seller_auth):
    ...
    assert str(log_entry['user_id']) == str(MOCK_SELLER_ID)
    assert log_entry['event_type'] == LogEventType.ORDER_STATUS_CHANGE
    assert f"Order {order_id} delivered by seller {MOCK_SELLER_ID}" in log_entry['message']
...
def test_deliver_order_invalid_status(seller_auth):
    ...
    assert log_entry['event_type'] == LogEventType.ORDER_DELIVER_FAIL
    # Check the specific log message from the ConflictError handler
    assert f"Failed to deliver order: {error_message}" in log_entry['message'] # <<< THIS MIGHT FAIL (NameError)
