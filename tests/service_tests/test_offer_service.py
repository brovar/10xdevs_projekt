...
from src.models import CategoryModel, OfferModel, UserModel
from src.schemas import LogEventType, OfferStatus, OfferSummaryDTO, UserRole
    ...
    try:
        ...
        if check_exception_attributes:
            for (
                attr_name,
                expected_value,
            ) in check_exception_attributes.items():
                if attr_name == "message":
                    # Special case for checking the exception message
                    assert expected_value in str(
                        e
                    ), f"Expected '{expected_value}' in exception message, got '{str(e)}'"
                else:
                    assert hasattr(
                        e, attr_name
                    ), f"Exception does not have attribute '{attr_name}'"
...
def offer_service(
    MockLogService, MockFileService, mock_db_session, mock_logger
):
...
async def test_create_offer_success_minimal(
    ...
    assert (
        type(result_dto).__name__ == OfferSummaryDTO.__name__
    )  # Check by class name
    ...
    assert (
        type(added_instance).__name__ == OfferModel.__name__
    )  # Check by class name
...
@pytest.mark.asyncio
async def test_create_offer_success_with_image(
    offer_service,
    mock_db_session,
    mock_seller_user,
    mock_category,
    ...
    offer_service.file_service.save_image.return_value = (
    ...
    offer_service.file_service.save_image.assert_awaited_once_with(
        mock_upload_file
    )
    mock_db_session.commit.assert_awaited_once()
...
@pytest.mark.asyncio
async def test_create_offer_invalid_price(
    offer_service, mock_seller_user, mock_category
):
...
@pytest.mark.asyncio
async def test_create_offer_invalid_quantity(
    offer_service, mock_seller_user, mock_category
):
...
@pytest.mark.asyncio
async def test_create_offer_category_not_found(
    offer_service, mock_db_session, mock_seller_user
):
...
@pytest.mark.asyncio
async def test_create_offer_db_commit_exception(
    offer_service, mock_db_session, mock_seller_user, mock_category
):
...
@pytest.mark.asyncio
async def test_create_offer_file_service_exception(
    offer_service,
    mock_db_session,
    mock_seller_user,
    mock_category,
    ...
    offer_service.file_service.save_image.side_effect = Exception(
        "File save error"
    )
...
@pytest.mark.asyncio
async def test_deactivate_offer_success(
    offer_service,
    mock_db_session,
    mock_seller_user,
    mock_offer_model,
    ...
    assert result_dto.status == OfferStatus.INACTIVE
    assert (
        offer_to_deactivate.status == OfferStatus.INACTIVE
...
@pytest.mark.asyncio
async def test_deactivate_offer_user_not_seller(
    offer_service, mock_offer_model
):
...
@pytest.mark.asyncio
async def test_deactivate_offer_not_found(
    offer_service, mock_db_session, mock_seller_user
):
...
@pytest.mark.asyncio
async def test_deactivate_offer_not_owner(
    offer_service, mock_db_session, mock_offer_model, mock_seller_user
):
...
@pytest.mark.asyncio
async def test_deactivate_offer_already_inactive(
    offer_service, mock_db_session, mock_offer_model, mock_seller_user
):
...
@pytest.mark.asyncio
async def test_deactivate_offer_invalid_status(
    offer_service, mock_db_session, mock_offer_model, mock_seller_user
):
...
@pytest.mark.asyncio
async def test_deactivate_offer_db_commit_exception(
    offer_service, mock_db_session, mock_seller_user, mock_offer_model
):
...
@pytest.mark.asyncio
async def test_mark_offer_as_sold_success(
    offer_service,
    mock_db_session,
    mock_seller_user,
    mock_offer_model,
...
@pytest.mark.asyncio
async def test_mark_offer_as_sold_user_not_seller(
    offer_service, mock_offer_model
):
...
@pytest.mark.asyncio
async def test_mark_offer_as_sold_not_found(
    offer_service, mock_db_session, mock_seller_user
):
...
@pytest.mark.asyncio
async def test_mark_offer_as_sold_not_owner(
    offer_service, mock_db_session, mock_offer_model, mock_seller_user
):
...
@pytest.mark.asyncio
async def test_mark_offer_as_sold_already_sold(
    offer_service, mock_db_session, mock_offer_model, mock_seller_user
):
...
@pytest.mark.asyncio
async def test_mark_offer_as_sold_invalid_status_archived(
    offer_service, mock_db_session, mock_offer_model, mock_seller_user
):
...
@pytest.mark.asyncio
async def test_mark_offer_as_sold_db_commit_exception(
    offer_service, mock_db_session, mock_seller_user, mock_offer_model
):
...
@pytest.mark.asyncio
async def test_list_all_offers_no_filters_default_sort(
    ...
    assert str(response.items[0].id) == str(
        offer1_id
...
@pytest.mark.asyncio
async def test_list_all_offers_with_search_filter(
    offer_service, mock_db_session
):
    """Test listing offers with a search term."""
    search_term = "Alpha"
    # Mocking execute to simulate filtering
    filtered_offer = OfferModel(
        id=uuid4(),
        title="Offer Alpha",
        seller_id=uuid4(),
        category_id=1,
...
@pytest.mark.asyncio
async def test_list_all_offers_with_category_filter(
    offer_service, mock_db_session
):
    """Test listing offers filtered by category_id."""
    category_id_to_filter = 1
    filtered_offer_cat = OfferModel(
        id=uuid4(),
        title="Offer Cat1",
        seller_id=uuid4(),
        category_id=category_id_to_filter,
        price=Decimal(10),
        quantity=1,
        status=OfferStatus.ACTIVE,
        created_at=datetime.now(timezone.utc),
    ...
    response = await offer_service.list_all_offers(
        category_id=category_id_to_filter
    )
    assert response.total == 1
    assert len(response.items) == 1
...
@pytest.mark.asyncio
async def test_list_all_offers_with_status_filter(
    offer_service, mock_db_session
):
    """Test listing offers filtered by status."""
    status_to_filter = OfferStatus.INACTIVE
    filtered_offer_status = OfferModel(
        id=uuid4(),
...
@pytest.mark.asyncio
async def test_moderate_offer_success(
    offer_service,
    mock_db_session,
    mock_offer_model,
    mock_seller_user,
    ...
    offer_to_moderate.status = (
        OfferStatus.ACTIVE
    )  # Can be any non-moderated status
    ...
    result_dto = await offer_service.moderate_offer(
        offer_id=offer_to_moderate.id
    )
...
@pytest.mark.asyncio
async def test_moderate_offer_already_moderated(
    offer_service, mock_db_session, mock_offer_model
):
...
@pytest.mark.asyncio
async def test_moderate_offer_db_commit_exception(
    offer_service, mock_db_session, mock_offer_model
):
    ...
    await assert_raises_exception(
        func=lambda: offer_service.moderate_offer(
            offer_id=mock_offer_model.id
        ),
        expected_exception=OfferModificationFailedException,
        check_exception_attributes={
...
@pytest.mark.asyncio
async def test_unmoderate_offer_success(
    offer_service,
    mock_db_session,
    mock_offer_model,
    mock_seller_user,
    ...
    result_dto = await offer_service.unmoderate_offer(
        offer_id=offer_to_unmoderate.id
    )

    assert (
        result_dto.status == OfferStatus.INACTIVE
    )  # Unmoderation sets to INACTIVE
...
@pytest.mark.asyncio
async def test_unmoderate_offer_not_moderated(
    offer_service, mock_db_session, mock_offer_model
):
