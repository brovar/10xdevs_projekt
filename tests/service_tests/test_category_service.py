...
@pytest.fixture
def category_service(mock_db_session: AsyncSession, mock_logger: Logger) -> CategoryService:
...
@pytest.mark.asyncio
async def test_category_service_initialization(category_service: CategoryService, mock_db_session: AsyncSession, mock_logger: Logger):
...
@pytest.mark.asyncio
async def test_get_all_categories_success(category_service: CategoryService, mock_db_session: AsyncSession):
    ...
    mock_db_session.execute.return_value.scalars.return_value.all.return_value = mock_categories_models
...
@pytest.mark.asyncio
async def test_get_all_categories_ordered_by_id(category_service: CategoryService, mock_db_session: AsyncSession):
    ...
    mock_db_session.execute.return_value.scalars.return_value.all.return_value = ordered_mock_models
...
@pytest.mark.asyncio
async def test_get_all_categories_empty(category_service: CategoryService, mock_db_session: AsyncSession):
...
@pytest.mark.asyncio
async def test_get_all_categories_db_exception(category_service: CategoryService, mock_db_session: AsyncSession, mock_logger: MagicMock):
    ...
    assert exc_info.value.detail["message"] == "Failed to retrieve category data"
    
    mock_logger.error.assert_called_once_with(f"Error fetching categories: {db_error_message}") 