...
# Add src directory to path FIRST to allow subsequent imports
sys.path.insert(
...
async def mock_get_offer(model_class, pk):
    ...
    return MockOffer(id=pk, seller_id=MOCK_SELLER_ID, status=offer_status)
...
def test_app():
    ...
    async def get_offer_image(
        ...
        try:
            ...
            logger.error(
                f"Unexpected error in get_offer_image endpoint: {str(exc)}"
            )
            return JSONResponse(
            ...
            logger.error(
                f"Unexpected error in get_offer_image endpoint: {str(e)}"
            )
            return JSONResponse(
...
def test_get_offer_image_invalid_filename(test_app):
    ...
    response = test_app.get(
        f"/media/offers/{MOCK_OFFER_ID}/invalid..filename.png"
    )
...
def test_get_offer_image_unexpected_error(test_app):
    ...
    StubMediaService._raise_on_check_access = Exception(
