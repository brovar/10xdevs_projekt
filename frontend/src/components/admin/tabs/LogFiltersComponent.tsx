  ...
  const validateDateRange = (): boolean => {
    ...
    if (start_date && end_date) {
      ...
      if (endDateTime < startDateTime) {
        setDateError('Data końcowa nie może być wcześniejsza niż data początkowa');
        return false;
      }
    }
  ...
  return (
    <div className="card mb-4">
      <div className="card-body">
        <h5 className="card-title">Filtry logów</h5>
        <form onSubmit={handleSubmit}>
          <div className="row g-3">
            {/* Event Type filter */}
            <div className="col-12 col-md-4">
              <label htmlFor="event_type" className="form-label">Typ zdarzenia</label>
              <select
              ...
              >
                <option value="">Wszystkie typy zdarzeń</option>
                {Object.values(LogEventType).map(type => (
                  <option key={type} value={type}>{type}</option>
                ))}
              </select>
              <div id="eventTypeHelp" className="form-text">Filtruj logi według typu zdarzenia</div>
            </div>

            {/* User ID filter */}
            <div className="col-12 col-md-4">
              <label htmlFor="user_id" className="form-label">ID Użytkownika</label>
              <input
                type="text"
              ...
              />
              <div id="userIdHelp" className="form-text">Logi dotyczące konkretnego użytkownika</div>
            </div>

            {/* IP Address filter */}
            <div className="col-12 col-md-4">
              <label htmlFor="ip_address" className="form-label">Adres IP</label>
              <input
                type="text"
                className="form-control"
                id="ip_address"
              ...
              />
              <div id="ipAddressHelp" className="form-text">Logi z określonego adresu IP</div>
            </div>

            {/* Start Date filter */}
            <div className="col-12 col-md-6">
              <label htmlFor="start_date" className="form-label">Data początkowa</label>
              <input
                type="datetime-local"
                className="form-control"
                id="start_date"
              ...
              />
              <div id="startDateHelp" className="form-text">Logi od tej daty i godziny</div>
            </div>

            {/* End Date filter */}
            <div className="col-12 col-md-6">
              <label htmlFor="end_date" className="form-label">Data końcowa</label>
              <input
                type="datetime-local"
                className="form-control"
                id="end_date"
              ...
              />
              <div id="endDateHelp" className="form-text">Logi do tej daty i godziny</div>
            </div>
