from fastapi import APIRouter, Depends, Request, status, Query, HTTPException, Path
...
from dependencies import get_db_session, get_logger, require_roles, require_authenticated, get_order_service, get_log_service
from services.order_service import OrderService
from schemas import UserRole, CreateOrderRequest, CreateOrderResponse, OrderListResponse, LogEventType, OrderDetailDTO, ErrorResponse
...
async def create_order(
    ...
    try:
        ...
        logger.error(f"Failed to create order for user {current_user['user_id'] if current_user else 'unknown'}: {e}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Failed to log order creation error: {log_error}", exc_info=True)
        
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error_code": "CREATE_ORDER_FAILED", "message": "Failed to create order"}
        )
...
async def list_buyer_orders(
    ...
    try:
        ...
        # Log the error
        logger.error(f"Error fetching orders for user {current_user['user_id']}: {str(e)}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Failed to log order list error: {str(log_error)}", exc_info=True)
...
async def get_order_details(
    ...
    try:
        ...
        # Log the error
        logger.error(f"Error fetching order details for order {order_id}, user {current_user['user_id']}: {str(e)}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Failed to log order details error: {str(log_error)}", exc_info=True)
...
@router.post(
    ...
    responses={
        ...
        400: {"model": ErrorResponse, "description": "Invalid order ID format"},
        401: {"model": ErrorResponse, "description": "User not authenticated"},
        403: {"model": ErrorResponse, "description": "User not authorized"},
        404: {"model": ErrorResponse, "description": "Order not found"},
...
async def ship_order(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to log order ship success: {log_error}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Failed to log order ship failure (ValueError): {log_error}", exc_info=True)
        raise HTTPException(
        ...
        try:
            ...
            logger.error(f"Failed to log order ship failure (PermissionError): {log_error}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"error_code": "INSUFFICIENT_PERMISSIONS", "message": str(e)}
        )
        ...
        try:
            ...
            logger.error(f"Failed to log order ship failure (ConflictError): {log_error}", exc_info=True)
        raise HTTPException(
        ...
        try:
            ...
            logger.error(f"Failed to log order ship failure (Exception): {log_error}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error_code": "SHIP_FAILED", "message": "Failed to ship order"}
        )
...
async def deliver_order(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to log order deliver success: {log_error}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Failed to log order deliver failure (ValueError): {log_error}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={"error_code": "ORDER_NOT_FOUND", "message": str(e)}
        )
    except PermissionError as e:
        logger.warning(f"Permission denied for delivering order {order_id}: {e}")
        # Log failure event
        try:
            await log_service.create_log(
                event_type=LogEventType.ORDER_DELIVER_FAIL,
            ...
            logger.error(f"Failed to log order deliver failure (PermissionError): {log_error}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"error_code": "INSUFFICIENT_PERMISSIONS", "message": str(e)}
        )
        ...
        try:
            ...
            logger.error(f"Failed to log order deliver failure (ConflictError): {log_error}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail={"error_code": "INVALID_ORDER_STATUS", "message": str(e)}
        )
        ...
        try:
            ...
            logger.error(f"Failed to log order deliver failure (Exception): {log_error}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error_code": "DELIVER_FAILED", "message": "Failed to deliver order"}
        ) 