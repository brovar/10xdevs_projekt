...
from dependencies import (get_db_session, get_logger, get_order_service,
                          require_seller)
from models import LogEventType, OfferModel
from schemas import OfferStatus, OfferSummaryDTO, OrderListResponse
...
async def get_seller_status(
    ...
    return {
        ...
        "metrics": {"total_offers": 0, "active_offers": 0, "sold_items": 0},
    }
...
async def list_seller_sales(
    ...
    sort: str = Query(
        "created_at_desc",
        description="Sort order (created_at_desc, created_at_asc, updated_at_desc, updated_at_asc, total_amount_desc, total_amount_asc, status_desc, status_asc)",
    ),
    ...
    try:
        ...
        result = await order_service.get_seller_sales(
            seller_id=seller_id, page=page, limit=limit, sort=sort
        )
        return result
...
async def list_seller_offers(
    ...
    search: Optional[str] = Query(
        None, description="Search term for offer title and description"
    ),
    category_id: Optional[int] = Query(
        None, description="Filter by category ID"
    ),
    status: Optional[OfferStatus] = Query(
        None, description="Filter by offer status"
    ),
    sort: Optional[str] = Query(
        "created_at_desc",
        description="Sort order (price_asc, price_desc, created_at_desc)",
    ),
    db_session: AsyncSession = Depends(get_db_session),
    logger: Logger = Depends(get_logger),
):
