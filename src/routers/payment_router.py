...
from dependencies import get_log_service, get_logger, get_payment_service
from schemas import LogEventType, PaymentCallbackResponse, TransactionStatus
                    ...
                    "examples": {
                        "TRANSACTION_NOT_FOUND": {
                            ...
                            "value": {
                                ...
                                "message": "Transaction 123e4567-e89b-12d3-a456-426614174000 not found",
                            },
                        },
                        "ORDER_NOT_FOUND": {
                            ...
                            "value": {
                                ...
                                "message": "Order not found for transaction 123e4567-e89b-12d3-a456-426614174000",
                            },
                        },
                    }
                }
...
async def handle_payment_callback(
    ...
    status_str: Optional[str] = Query(
        None, description="Transaction status: success, fail, or cancelled"
    ),
    ...
    try:
        ...
        result = await payment_service.process_payment_callback(
            transaction_id=transaction_id, status=transaction_status
        )
        # Log success
        await log_service.create_log(
        ...
        return PaymentCallbackResponse(
            message="Callback processed", order_status=result.order_status
        )
