...
async def create_test_data():
    ...
    async with async_session_maker() as session:
        ...
        for i, title in enumerate(game_titles):
            ...
            quantity = (
                0 if status_variations[i] == OfferStatus.SOLD else (i % 10) + 1
            )
        ...
        for i, (created_at, status) in enumerate(
            zip(time_frames, order_statuses)
        ):
            order_id = uuid.uuid4()
        ...
        for i, order in enumerate(orders):
            ...
            log = LogModel(
                ...
                timestamp=time_frames[
                    i
                ],  # Use the same timestamp as the order
            )
            logs.append(log)
        ...
        logger.info(
            f"Created {len(orders)} sample orders for buyer@steambay.com."
        )
        logger.info(
            f"Created test users: admin@steambay.com, seller@steambay.com, buyer@steambay.com (Password for all: matching role + 123!)"
        )
