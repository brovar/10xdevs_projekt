...
from schemas import LoginUserRequest, UserStatus, LogEventType, UserDTO, UpdateUserRequest, ChangePasswordRequest, UserRole, UserListResponse, OfferStatus, OrderStatus
from models import UserModel, LogModel, OfferModel, OrderModel, OrderItemModel
...
class UserService:
    ...
    async def list_users(
        ...
        # Apply filters
        query = base_query.where(*filters).order_by(UserModel.created_at.desc())
        ...
        users_result = await self.db_session.execute(query.offset(offset).limit(limit))
        ...
        try:
            ...
            async with self.db_session.begin():
                ...
                update_query = (
                    update(UserModel)
                    .where(UserModel.id == user_id)
                    .values(status=UserStatus.INACTIVE, updated_at=current_time)
                    .returning(
                        UserModel.id, UserModel.email, UserModel.role, UserModel.status,
                        UserModel.first_name, UserModel.last_name, UserModel.created_at, UserModel.updated_at
                    )
                )
                ...
                if user.role == UserRole.SELLER:
                    ...
                    offers_res = await self.db_session.execute(
                        select(OfferModel.id).where(
                            ...
                            OfferModel.status.in_([OfferStatus.ACTIVE, OfferStatus.INACTIVE])
                        )
                    )
                    ...
                    if order_ids:
                        await self.db_session.execute(
                            ...
                            .values(status=OrderStatus.CANCELLED, updated_at=current_time)
                        )
            return updated_user
        except ValueError:
