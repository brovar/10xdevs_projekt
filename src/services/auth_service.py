    ...
    async def register_user(self, register_data: RegisterUserRequest, request: Request) -> UserModel:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Integrity error while registering user {normalized_email}")
                raise AuthServiceError(
                    error_code="EMAIL_ALREADY_EXISTS",
                    message="Użytkownik o podanym adresie email już istnieje.",
                    status_code=400
        ...
        try:
            ...
            try:
                ...
                if self.session_service is None:
                    self.logger.error("Session service is None in login_user method")
                    raise AuthServiceError(
                ...
                if not hasattr(self.session_service, 'create_session') or not callable(self.session_service.create_session):
                    self.logger.error("Session service doesn't have a callable create_session method")
                    raise AuthServiceError(
                        error_code="SESSION_CREATION_FAILED",
                        message="Wystąpił błąd podczas tworzenia sesji. Spróbuj ponownie później.",
                        status_code=500
                ...
                self.logger.info(f"Creating session for user {user.id} with role {user.role.value}")
                try:
                    ...
                    self.logger.info(f"Session created successfully for user {user.id}")
                except Exception as session_error:
                    self.logger.error(f"Error creating session: {str(session_error)}")
                    self.logger.error(f"Error type: {type(session_error)}")
                    self.logger.error(f"Session service: {self.session_service}")
                    self.logger.error(f"Session methods: {dir(self.session_service)}")
                    raise AuthServiceError(
                        error_code="SESSION_CREATION_FAILED",
                        message="Wystąpił błąd podczas tworzenia sesji. Spróbuj ponownie później.",
                        status_code=500
