...
class OrderService:
    ...
    async def get_buyer_orders(
        ...
        try:
            ...
            count_query = select(func.count()).select_from(OrderModel).where(OrderModel.buyer_id == buyer_id)
            ...
            query = (
                select(
                    OrderModel,
                    func.sum(OrderItemModel.price_at_purchase * OrderItemModel.quantity).label("total_amount")
                )
    ...
    async def get_order_details(
        ...
        try:
            ...
            if user_role == UserRole.BUYER:
                ...
                seller_check_result = await self.db_session.execute(seller_check_query)
    ...
    async def ship_order(
        ...
        if order.status != OrderStatus.PROCESSING:
            raise ConflictError("Order must be in 'processing' status to be shipped")
        ...
        seller_items_count = (await self.db_session.execute(seller_check_query)).scalar() or 0
        if seller_items_count == 0:
    ...
    async def deliver_order(
        ...
        if order.status != OrderStatus.SHIPPED:
            raise ConflictError("Order must be in 'shipped' status to be delivered")
    ...
    async def get_admin_orders(
        ...
        result = await self.db_session.execute(
            base_query.order_by(OrderModel.created_at.desc()).offset(offset).limit(limit)
        )
        ...
        for order in orders:
            ...
            # Calculate total amount
            total_amount = sum(item.quantity * item.price_at_purchase for item in items)
            summaries.append(
                OrderSummaryDTO(
                    id=order.id,
                    status=order.status,
    ...
    async def cancel_order(self, order_id: UUID) -> OrderDetailDTO:
        ...
        if order.status == OrderStatus.DELIVERED:
            raise ValueError(f"Order {order_id} is already delivered and cannot be cancelled")
        ...
        return await self.get_order_details(order_id, order.buyer_id, UserRole.ADMIN) 