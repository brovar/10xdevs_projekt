...
class OrderService:
    ...
    async def get_order_details(
        ...
        try:
            ...
            self.logger.info(
                f"get_order_details called - order_id: {order_id}, user_id: {user_id}, user_role: {user_role}"
            )
            ...
            self.logger.info(
                f"Order found: {order.id}, buyer_id: {order.buyer_id}"
            )
            ...
            if user_role == UserRole.BUYER:
                ...
                self.logger.debug(
                    f"Comparing buyer_id: '{str_order_buyer_id}' with user_id: '{str_user_id}'"
                )

                if str_order_buyer_id != str_user_id:
                    self.logger.warning(
                        f"Access denied - buyer {str_user_id} is not owner of order {order_id}"
                    )
                    self.logger.debug(
                        f"Types: order.buyer_id: {type(order.buyer_id)}, user_id: {type(user_id)}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail={
                ...
                self.logger.info(
                    f"Buyer {str_user_id} access granted for order {order_id}"
                )
                ...
                if seller_items_count == 0:
                    self.logger.warning(
                        f"Access denied - seller {user_id} has no items in order {order_id}"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_403_FORBIDDEN,
                        detail={
                ...
                self.logger.info(
                    f"Seller {user_id} access granted for order {order_id}"
                )

            # For ADMIN role, no additional permission check is needed
            else:
                self.logger.info(
                    f"Admin access granted for user {user_id} to order {order_id}"
                )
    ...
    async def get_seller_sales(
        ...
        try:
            ...
            if sort == "created_at_asc":
                ...
                order_by_clause = func.sum(
                    OrderItemModel.price_at_purchase * OrderItemModel.quantity
                ).asc()
